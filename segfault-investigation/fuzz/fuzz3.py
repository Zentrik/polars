...
seed = 0; nops = 0; logf = None; rng = random.Random(0)
...
def rand_series(name, n, depth=0, kind=None):
    if kind is None: kind = rng.choice(["i8","i32","i64","u8","u64","f32","f64","str","bool","date","datetime","datetime_tz","duration","time","cat","enum","dec","list","struct","array","null","binary","i128"] if depth < 2 else ["i32","f64","str","bool"])
    ...
    if kind == "i8": data=[nul(rng.randint(-128,127)) for _ in range(n)]; s=pl.Series(name,data,pl.Int8)
        ...
        ik = rng.choice(["i32","f64","str","bool","cat","date","datetime","dec","list","struct","i8","u64","binary","null"]) if depth < 1 else rng.choice(["i32","f64","str","bool"])
        inner = rand_series("i", 0, depth+1, ik).dtype
        data=[nul(rand_series("i", rng.choice([0,1,2,5,20]), depth+1, ik).to_list()) for _ in range(n)]
        s=pl.Series(name,data,pl.List(inner))
    elif kind == "array":
        w = rng.choice([1,2,3])
        ik = rng.choice(["i32","f64","str","bool","cat","date","dec","list","struct","i8","u64"]) if depth < 1 else rng.choice(["i32","f64","str","bool"])
        inner = rand_series("i", 0, depth+1, ik).dtype
        data=[nul(rand_series("i", w, depth+1, ik).to_list()) for _ in range(n)]
...
def rand_df(n=None, ncols=None):
    ...
    for _ in range(20):
        try:
            ...
            log(f"  rand_df exc {type(ex).__name__}: {str(ex)[:150]}")
    return pl.DataFrame({"c0": list(range(n))})
...
DUMP = os.environ.get("FUZZ_DUMP")
def op(df, i):
    choice = rng.choice(OPS)
    log(f"op#{i}={choice.__name__} schema={df.schema} h={df.height} chunks={df.n_chunks('all')}")
    if DUMP:
        try: df.write_ipc(DUMP, compression="uncompressed")
        except Exception as ex: log(f"  dump failed {ex}")
    return choice(df)
...
def op_rolling_groupby(df):
    ...
    try:
        d = df.sort(c)
        if rng.random()<0.5:
            closed = rng.choice(["left","right","both","none"])
            log(f"  rolling(index={c!r}, period={period!r}, closed={closed!r}) aggs={aggs}")
            return collect(d.lazy().rolling(index_column=c, period=period, closed=closed).agg(aggs))
        else:
            per = rng.choice([None, period]); closed = rng.choice(["left","right","both","none"]); label = rng.choice(["left","right","datapoint"]); ib = rng.random()<0.5
            log(f"  group_by_dynamic(index={c!r}, every={period!r}, period={per!r}, closed={closed!r}, label={label!r}, include_boundaries={ib}) aggs={aggs}")
            return collect(d.lazy().group_by_dynamic(index_column=c, every=period, period=per, closed=closed, label=label, include_boundaries=ib).agg(aggs))
    except pl.exceptions.PolarsError: return df
...
def main(seed_, nops_, logpath):
    global seed, nops, logf, rng
    seed = seed_; nops = nops_; logf = open(logpath, "a", buffering=1); rng = random.Random(seed)
    df = rand_df()
    ...
    for i in range(nops):
        ...
        try:
            ...
            if isinstance(out, pl.DataFrame) and out.width>0:
                badlen = [(c, out[c].len()) for c in out.columns if out[c].len() != out.height or sum(out[c].chunk_lengths()) != out[c].len()]
                if badlen:
                    log(f"  INVALID FRAME: height={out.height} bad={badlen[:5]}")
                    out = None
                else:
