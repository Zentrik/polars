...
// ---------------------------------------------------------------------------
// Shims for the two other polars-internal symbols the async_executor uses.
// The executor source (async_executor/{mod,task,park_group}.rs) is copied
// byte-for-byte from py-1.37.1; only its `use` lines for these symbols were
// repointed here so we can build it standalone against crossbeam-deque 0.8.6.
// ---------------------------------------------------------------------------
use std::cell::Cell;

thread_local! {
    /// Mirror of polars_core::ALLOW_RAYON_THREADS. The executor only ever
    /// calls `.set(false)` on it; behaviour is irrelevant to the work-stealing
    /// paths under test.
    pub static ALLOW_RAYON_THREADS: Cell<bool> = const { Cell::new(true) };
}

/// Mirror of polars_error::signals::try_raise_keyboard_interrupt. In real
/// polars this may raise a Python KeyboardInterrupt to unwind a task; here we
/// no-op (cancellation is driven explicitly via CancelHandle in the harness).
#[inline(always)]
